...
| サービス | 用途 |
|----------|------|
| **Firebase Firestore** | メインDB（ゲーム状態、履歴、統計、お題）※チャットはPhase2 |
| **Firebase Storage** | スケッチ画像保存 |
| **Firebase Hosting** | 静的サイトホスティング |
| **Firebase Cloud Functions** | Gemini API連携 |

**補足:** Firebase Authentication は Phase2 以降で導入予定（MVPは簡易ニックネーム運用）。

## 開発・運用の補足

- **Firestore リスナー（onSnapshot）**: 状態変化が重要な画面のみ使用（基本は1回取得）。
- **Firebase Emulator**: 開発はエミュレータで進め、本番接続はMVP公開直前。
